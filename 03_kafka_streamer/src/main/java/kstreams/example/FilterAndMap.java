...
public class FilterAndMap extends StreamApp {

    public static void main(String[] args) throws Exception {
        // Configure the Streams application.
        final Properties streamsConfiguration = getStreamProperties("sensor-filter");
    ...
    private static void buildStream(StreamsBuilder builder) {
        ...
        KStream<String, String> filteredStream =
                inputStream.filter((key, value) -> value.getSensorData1() > 0.5)
                           .mapValues(value -> value.getSensorData2() > 0.2 ? "broken" : "ok");

        filteredStream.to("sensor-data-output", Produced.with(Serdes.String(), Serdes.String()));
    }
}
